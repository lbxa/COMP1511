...
int main (int argc, char *argv[]) {
   ...
   int fullRow = SIZE * BYTES_PER_PIXEL * squareSize;
   int EVEN_ROW = 0;
   int EVEN_BYTE = 0;

   if (squareSize != SIZE) {
      // total amount of bytes being written into the file
      while (pos < numBytes) {

         // keeping track of whether square is black or white
         while (pos % bytesPerSquare != EVEN_BYTE) {
            fwrite(&byte, sizeof(byte), 1, outputFile);
            pos++;
         }

         if (pos % fullRow != EVEN_ROW) {
            // swap colors around for every ODD row
            if (byte == WHITE) {
               byte = BLACK;
            } else if (byte == BLACK) {
               byte = WHITE;
      ...
      // when user chooses 512 to print out one big block
      while (pos < numBytes) {
         byte = WHITE;
         fwrite(&byte, sizeof(byte), 1, outputFile);
         pos++;
